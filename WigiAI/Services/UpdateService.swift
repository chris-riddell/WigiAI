...
private class SilentUpdaterDelegate: NSObject, SPUUpdaterDelegate {
    func updater(_ updater: SPUUpdater, didFinishLoading appcast: SUAppcast) {
        LoggerService.updates.debug("📦 Appcast loaded successfully")
    }

    func updater(_ updater: SPUUpdater, didAbortWithError error: Error) {
        LoggerService.updates.warning("⚠️ Update check aborted: \(error.localizedDescription)")
        // Don't show dialog - just log
    }

    func updaterDidNotFindUpdate(_ updater: SPUUpdater) {
        LoggerService.updates.debug("✅ No updates available")
    }

    func updater(_ updater: SPUUpdater, failedToDownloadUpdate item: SUAppcastItem, error: Error) {
        LoggerService.updates.warning("⚠️ Failed to download update: \(error.localizedDescription)")
        // Don't show dialog - just log
    }
}
...
class UpdateService: ObservableObject {
    ...
    private func setupUpdater() {
        ...
        if let updaterController = updaterController {
            ...
            LoggerService.updates.info("✅ UpdateService: Sparkle updater initialized successfully")
    ...
    func configureFromSettings() {
        guard let updater = updaterController?.updater else {
            LoggerService.updates.warning("⚠️ UpdateService: Updater not available")
            return
        }
        ...
        LoggerService.app.debug("📝 UpdateService: Configured - Auto-check: \(settings.autoUpdateEnabled), Interval: 24 hours")
    }
    ...
    func checkForUpdatesInBackground() {
        guard let updater = updaterController?.updater else {
            LoggerService.updates.warning("⚠️ UpdateService: Cannot check for updates - updater not available")
            return
        }
        ...
        LoggerService.app.debug("🔍 UpdateService: Checking for updates in background...")
    }
    ...
    func checkForUpdates() {
        guard let updaterController = updaterController else {
            LoggerService.updates.error("❌ UpdateService: Cannot check for updates - controller not available")
        ...
        guard updater.canCheckForUpdates else {
            LoggerService.updates.error("❌ UpdateService: Updater reports it cannot check for updates")
        ...
        LoggerService.app.debug("🔍 UpdateService: Checking for updates (with UI)...")
    ...
    func setAutoUpdateEnabled(_ enabled: Bool) {
        ...
        LoggerService.app.debug("📝 UpdateService: Automatic updates \(enabled ? "enabled" : "disabled") - checking daily")
    }
